let {TreeTool, UuidTool, CommonTool} = require('./index')
...
const instance = TreeTool.createInstance({pid: 'parentId'})

// 列表转树 不创建实例
function testFromList() {
    const list = getList()
    const tree = TreeTool.fromList(list, {pid: 'parentId'})
...
console.log(UuidTool.getUuid())
console.log(UuidTool.getUuidByLenAndRadix(16))
console.log(CommonTool.string.firstUpperCase('abc'))
